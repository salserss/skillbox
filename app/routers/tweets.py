...
from app.schemas.tweet_schema import TweetCreate, TweetIn, TweetOut
...
async def create_tweet(tweet_in: TweetIn, current_user, session: AsyncSession = Depends(async_get_db),):
    new_tweet = Tweet(user_id=current_user.id, tweet_data=tweet_in.tweet_data,)
...
@router.delete("/tweets/{tweet_id}", status_code=status.HTTP_200_OK, response_model=DefaultSchema,)
async def delete_tweet(tweet_id: int, current_user, session: AsyncSession = Depends(async_get_db),):
...
@router.post("/tweets/{tweet_id}/likes", status_code=status.HTTP_201_CREATED, response_model=DefaultSchema,)
async def like_a_tweet(tweet_id: int, current_user, session: AsyncSession = Depends(async_get_db),):
    tweet_to_like = await get_tweet_by_id(tweet_id=tweet_id, session=session)
    like = await get_like_by_id(session=session, tweet_id=tweet_id, user_id=current_user.id)
    if not like:
...
@router.delete("/tweets/{tweet_id}/likes", status_code=status.HTTP_200_OK, response_model=DefaultSchema,)
async def delete_like_from_tweet(tweet_id: int, current_user, session: AsyncSession = Depends(async_get_db),):
    await session.commit()
    test_tweet = await get_tweet_by_id(tweet_id=tweet_id, session=session)
    like = await get_like_by_id(session, tweet_id=test_tweet.id, user_id=current_user.id)
    if like:
        await session.delete(like)
        await session.commit()
    else:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="You already do not like that tweet.",)
...
@router.get("/tweets/{user_id}", status_code=status.HTTP_200_OK, response_model=TweetOut,)
async def get_following_tweets(user_id: int, current_user, session: AsyncSession = Depends(async_get_db),):
